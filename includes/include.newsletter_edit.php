...
if ($action == "news_create" && $perm->have_perm_area_action($area, "news_create"))
{
	...
	$oNewsletter 	= $oNewsletters->create(i18n("-- new newsletter --", $plugin_name));
	...
	if ($oJob) {
		$notis = $notification->returnNotification("info", i18n("Newsletter dispatch job has been added for this newsletter", $plugin_name)) . "<br>";
	} else {
		$notis = $notification->returnNotification("error", i18n("Newsletter dispatch job has been not been added! Please check newsletter details", $plugin_name)) . "<br>";
	} 
	...
	if ($iTestIDNewsGroup == 0)
	{
		...
		if ($bSend) {
			...
			$aRecipients[] = i18n("None", $plugin_name);
		} 
	} else {
	...
	if ($bSend) {
		$notis = $notification->returnNotification("info", i18n("Test newsletter has been sent to:", $plugin_name) . "<br />" . implode("<br />", $aRecipients) . "<br />");
	} else {
		$notis = $notification->returnNotification("warning", i18n("Test newsletter has not been sent (partly or completely):", $plugin_name) . "<br />" .  
															  i18n("Successful:", $plugin_name) . "<br />" . implode("<br />", $aRecipients) . "<br />" .
															  i18n("Error messages:", $plugin_name) . "<br />" . $oNewsletter->_sError);
	}
} else {
...
if ($oNewsletter->virgin == false && $oNewsletter->get("idclient") == $client && $oNewsletter->get("idlang") == $lang)
{
	...
	if ($action == "news_save" && $perm->have_perm_area_action($area, $action))
	{
		...
		if ($oNewsletter->get("name") != $sName)
		{
			...
			$oNewsletters->query();
	 		...
	 		{
	 			$aMessages[] = i18n("Could not set new newsletter name: name already exists", $plugin_name);	
	 		} else {
	 			...
	 			if ($oNewsletter->get("idart") > 0)
	 			{
					...
					$oArticles->query();
	 				...
	 				{
	 					$oArticle->set("title", sprintf(i18n("Newsletter: %s", $plugin_name), $oNewsletter->get("name")));
	 					$oArticle->store();
	 				}
	 				unset ($oArticle);
	 				unset ($oArticles);
		...
		if ($_REQUEST["optSendTo"] == "selection" && !is_array($_REQUEST["selGroup"])) {
			$aMessages[] = i18n("'Send to recipients in selected groups' has been selected, but no group has been specified. Selection has been set to 'Send to all recipients'", $plugin_name);
	...
	$oForm->addHeader(i18n("Edit newsletter", $plugin_name));
	...
	$aItems   = array();
	$aItems[] = array("text", i18n("Text only", $plugin_name));
	if ($oClientLang->getProperty("newsletter", "html_newsletter") == "true") {
		$aItems[] = array("html", i18n("HTML and text", $plugin_name));		
	} else {
	...
	$oForm->add(i18n("Type", $plugin_name), $oSelType->render());
	...
	$oForm->add(i18n("From (E-Mail)", $plugin_name),	$oTxtFromEMail->render());
	$oForm->add(i18n("From (Name)", $plugin_name),	$oTxtFromName->render()."&nbsp;".i18n("optional", $plugin_name));
	$oForm->add(i18n("Subject", $plugin_name),		$oTxtSubject->render());
	...
	$oSelGroup->setAlt(i18n("Note: Hold <Ctrl> to select multiple items.", $plugin_name));
	...
	// Recipients	
	$oForm->add(i18n("Recipients", $plugin_name), 
			$oSendToAll->toHTML(false)."&nbsp;".i18n("Send newsletter to all recipients", $plugin_name)."<br />".chr(10).
			$oSendToDefault->toHTML(false)."&nbsp;".i18n("Send newsletter to the members of the default group", $plugin_name)."<br />".chr(10).
			$oSendToGroups->toHTML(false)."&nbsp;".i18n("Send newsletter to the members of the selected group(s):", $plugin_name)."<br />".chr(10).
			$oSelGroup->render());
	...
	$oSelFiles->setAlt(i18n("Note: Hold <Ctrl> to select multiple items.", $plugin_name));
	//$oSelGroup->autoFill($aItems);
	...
	$oForm->add(i18n("Attachments", $plugin_name), $oSelFiles->render().'<br /><span onclick="fncShowImageBrowser();">Test</span>');
	*/
	...
	if (getEffectiveSetting("newsletter", "option-cronjob-available", "false") == "true") {
		...
		$ckbCronJob->setAlt(i18n("Option has to be enabled as client setting - see techref for details", $plugin_name));
	}
	...
	$oTxtDispatchDelay->setAlt(i18n("Note: Set to 0 to send chunks manually.", $plugin_name));
	$oCkbSaveAsDefault	= new cHTMLCheckbox("ckbSetDefault", "1");
			
	$oForm->add(i18n("Options", $plugin_name),
			$ckbWelcome->toHTML(false)."&nbsp;".i18n("Welcome-Newsletter", $plugin_name)."<br />".
			$ckbCronJob->toHTML(false)."&nbsp;".i18n("Use cronjob", $plugin_name)."<br />".
			$oCkbDispatch->toHTML(false)."&nbsp;".i18n("Send in blocks:", $plugin_name)."&nbsp;&nbsp;&nbsp;".
			i18n("Recipients per block:", $plugin_name)."&nbsp;".$oTxtDispatchCount->render()."&nbsp;".
			i18n("Delay between blocks:", $plugin_name)."&nbsp;".$oTxtDispatchDelay->render()."&nbsp;".i18n("sec.", $plugin_name)."<br />".
			$oCkbSaveAsDefault->toHTML(false)."&nbsp;".i18n("Save option settings as default", $plugin_name));
		
	$oForm->add(i18n("Author", $plugin_name), $classuser->getUserName($oNewsletter->get("author")) . " (". $oNewsletter->get("created").")" ); 
	$oForm->add(i18n("Last modified by", $plugin_name), $classuser->getUserName($oNewsletter->get("modifiedby")). " (". $oNewsletter->get("modified").")" );	
